<?php
namespace Sts\Controllers\Admin;

use EmployeeRecoverEmail;
    ...
    * @var array|string|null $data Recebe os dados que devem ser enviados para VIEW */
    ...
    public function index() {
        ...
        if($_POST) {
            ...
            require_once(dirname(__FILE__, 3) . '/Views/email/EmployeeRecoverEmail.php');

            $Read = new \Sts\Models\Employee\Read();
            $email = $_POST['email'];

            // Buscando funcionario
            $employee = $Read->employeeEmail($email);

            // Checando se funcionario existe
            if(!$employee) {
                $_SESSION['return-email'] = $email;

                $_SESSION['msg'] = 'E-Mail inválido!';
                $_SESSION['msg-type'] = 'error';
                header("Location: $this->url/admin/recuperarsenha");
            ...
            $End->updateMyPassword($employee['id'], $cryptoPass);


            // Enviando senha para email do funcionario
            $title = 'Nova senha';

            /**
             * @var $Text -> Recebe a funcao create() da Classe EmployeeRecoverEmail()
             * Responsavel pelo conteudo do email
             * Diretorio da Classe: app/sts/Views/email/EmployeeRecoverEmail.php
             */
            $Text = new EmployeeRecoverEmail();
            $text = $Text->create($employee['firstName'], $password);
             ...
             */
            sendEmail($email, $employee['fullName'], $title, $text);
        ...
        $loadView = new \Core\ConfigView("sts/views/public/adminRecover", $this->data);
        $loadView->loadView();
    }
